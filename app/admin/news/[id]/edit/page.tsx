...
import { useAdminNewsItem, useUpdateNews } from '@/hooks/use-admin';
...
type NewsEditFormData = z.infer<typeof newsEditSchema>;
...
export default function AdminNewsEditPage() {
  ...
  const { data: newsItem, isLoading, isError } = useAdminNewsItem(newsId);
  ...
  const onSubmit = async (formData: NewsEditFormData) => {
    try {
  ...
  if (isError || (!isLoading && !newsItem)) {
    return (
      <div className="py-12 text-center text-destructive">
        뉴스/공지 정보를 불러올 수 없습니다.
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold tracking-tight">뉴스·공지 수정</h1>
      </div>
